              ...
              systemd.user.services.dude-agent = {
                ...
                Service = {
                  ...
                  ExecStartPre = [
                    ...
                    "${pkgs.bash}/bin/bash -c '${pkgs._1password-cli}/bin/op run --env-file ${cfg.opvarsFile} -- gh auth login --with-token \"$$GH_TOKEN\"'"
                    "${pkgs.bash}/bin/bash -c '${pkgs._1password-cli}/bin/op run --env-file ${cfg.opvarsFile} -- ob login --email=\"$$OB_EMAIL\" --password=\"$$OB_PASSWORD\"'"
                  ]
                  ++
